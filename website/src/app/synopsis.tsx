...
import {API_URL, BACKDROP_ITEMS, ENDING_ITEMS, GENRE_ITEMS, MAX_PRESET_LIST_LENGTH, MAX_SELECTED_PRESET_LENGTH} from "../util/constants";
...
function ChevronIcon({up, className, onClick}: ChevronIconProps) {
    return  up ? <ChevronUpIcon className={className} onClick={onClick} /> : <ChevronDownIcon className={className} onClick={onClick} />;
}

function SynopsisMain() {
    const [notify, setNotify] = useState<NotificationProps>({ type: "success", message: "" });
    const [selectedPresets, setSelectedPresets] = useState<Character[]>([]);
                ...
                <div className="w-[620px] flex flex-col gap-y-12">
                    <div>
                        ...
                        <div className="flex gap-x-[5px] overflow-x-auto">
                            {!dataLoading && presets.map((character, index) => {
                                return (
                                    <button key={`character_${index}`} className="flex flex-col items-center"
                                        ...
                                        <div className="w-[100px] h-[100px] rounded-xl shrink-0 overflow-hidden border-2 hover:border-indigo-600 relative">
                                            <img className="w-full h-full object-cover" src={`${API_URL}/images/${character.profile_image}`} />
                                            {character.selected &&
                        ...
                        <div className="flex flex-col gap-y-5">
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="w-full">
                                    <Input textArea placeholder="이벤트를 서술해주세요" value={eventDescription} onChange={(e) => { setEventDescription(e.target.value) }} disabled={synopsisLoading} />
                                </div>
                            </div>
                        </div>
                ...
                <div className="w-[340px] flex flex-col gap-y-6">
                            ...
                            return (
                              <>
                                  <h3 className="flex flex-row items-center">
                                      <ChevronIcon up={!synopsis.open} className="flex justify-center w-6 h-6 text-gray-400 hover:text-gray-500 hover:cursor-pointer" onClick={() => {
                    ...
                    <article className="flex flex-col prose">
                        <div className="flex justify-between mb-[14px]">
                            ...
                            <Button label="실행" onClick={() => handleRegenerateScenario()} loading={scenarioLoading} disabled={synopses.length == 0} />
                        </div>
