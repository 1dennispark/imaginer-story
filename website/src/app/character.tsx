...
function CharacterMain() {
    ...
    const getPresetData = async () => {
      try {
        ...
        if (data.length >= MAX_PRESET_LIST_LENGTH) {
          data = data.slice(MAX_PRESET_LIST_LENGTH);
        }
        setPresetList(data);
      } catch (e) {
        console.error(e);
  ...
  const handleGenerateCharacter = async () => {
    ...
    try {
      ...
      const {data: {context, profile_image}} = await apis.generateCharacter(newCharacter);

      const generatedCharacter = {...newCharacter, context, profile_image};
  ...
  const handleSavePreset = async () => {
    ...
    try {
      const {data} = await apis.addCharacter(generatedCharacter);
              ...
              {selectedImageList.map((image, index) => {
                return (
                  <button key={`selected_img_${index}`} className="w-[100px] h-[100px] rounded-xl shrink-0 overflow-hidden relative" onClick={() => handleDeleteImage(index)}>
                    <img className="w-full h-full object-cover" src={`${API_URL}/images/${image}`}/>
          ...
          <div>
            ...
            <div className="flex flex-col gap-y-5">
              <div className="flex items-center">
                ...
                <Input placeholder="캐릭터 이름을 입력해주세요" value={characterName ?? ''} onChange={(e) => {
              ...
              <div className="flex gap-x-[35px]">
                <div className="flex items-center">
                  ...
                  <Dropdown items={MBTI_ITEMS} placeholder="MBTI" selectedValue={selectedMBTI} onSelected={(item) => {
                    handleSelectDropdown('mbti', item)
                  }} disabled={apiLoading}/>
                </div>
                <div className="flex items-center">
                  <div className="w-[72px] text-base font-semibold shrink-0">성별</div>
                  <Dropdown items={GENDER_ITEMS} placeholder="성별" selectedValue={selectedGender} onSelected={(item) => {
                    handleSelectDropdown('gender', item)
                  }} disabled={apiLoading}/>
                </div>
                <div className="flex items-center">
                  <div className="w-[72px] text-base font-semibold shrink-0">나이대</div>
                  <Dropdown items={AGE_RANGE_ITEMS} placeholder="나이대" selectedValue={selectedAgeRange} onSelected={(item) => {
              ...
              <div className="flex items-start">
                ...
                <div className="w-full">
                  <Input textArea placeholder="캐릭터 설명을 입력해주세요" value={characterDescription ?? ''} onChange={(e) => {
        ...
        <div className="w-[340px] flex flex-col">
          ...
          <article className="prose">
            <div className="flex mb-5 items-center">
              {generatedCharacter?.profile_image &&
                <div className="flex flex-col items-center w-[100px]">
                  <img className="w-full h-[100px] object-cover rounded-xl" src={`${API_URL}/images/${generatedCharacter.profile_image}`} alt="" />
                </div>
              }
            ...
            <div className="font-normal text-base text-gray-900">
              <p>
                {generatedCharacter?.context?.split('\n').map(line => {
                  return (
                    <>
                      {line}<br/>
                    </>
